...
- **Global scope**: `D:/E-commerce/bazzar-mart/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/E-commerce/bazzar-mart/.claude/notes/`)
...
2. Glob `D:/E-commerce/bazzar-mart/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/E-commerce/bazzar-mart/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/E-commerce/bazzar-mart/.claude/notes/` — works in any directory
