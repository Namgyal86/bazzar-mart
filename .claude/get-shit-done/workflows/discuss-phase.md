...
<required_reading>
@D:/E-commerce/bazzar-mart/.claude/get-shit-done/references/domain-probes.md
@D:/E-commerce/bazzar-mart/.claude/get-shit-done/references/gate-prompts.md
@D:/E-commerce/bazzar-mart/.claude/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
```bash
INIT=$(node "D:/E-commerce/bazzar-mart/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "D:/E-commerce/bazzar-mart/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-advisor 2>/dev/null)
```
...
- Read and execute @D:/E-commerce/bazzar-mart/.claude/get-shit-done/workflows/discuss-phase-power.md end-to-end
...
```bash
TODO_MATCHES=$(node "D:/E-commerce/bazzar-mart/.claude/get-shit-done/bin/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "D:/E-commerce/bazzar-mart/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @D:/E-commerce/bazzar-mart/.claude/agents/gsd-advisor-researcher.md for your role and instructions.
...
```bash
MAX_PASSES=$(node "D:/E-commerce/bazzar-mart/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
node "D:/E-commerce/bazzar-mart/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "D:/E-commerce/bazzar-mart/.claude/get-shit-done/bin/gsd-tools.cjs" state record-session \
...
```bash
node "D:/E-commerce/bazzar-mart/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
**Full step-by-step instructions:** @D:/E-commerce/bazzar-mart/.claude/get-shit-done/workflows/discuss-phase-power.md
